...
public class StoreServlet extends HttpServlet {
    ...
    private void incPageCount(HttpServletRequest request) {
        int pagesVisited;
        HttpSession session = request.getSession();
        if((Integer)session.getAttribute("numPagesVisited") != null) {
            pagesVisited = (Integer)session.getAttribute("numPagesVisited");
            session.setAttribute("numPagesVisited", pagesVisited += 1);
        }
    }
    ...
    private void addToCart(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // TO DO: Add code to check if the productId exists in the product database
        if (!products.containsKey(productId)) {
            response.sendRedirect("shop");
            return;
        }
